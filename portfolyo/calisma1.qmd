...
**VERİ BİLİMİ VE ENDÜSTRİ MÜHENDİSLİĞİNDE VERİNİN KULLANIM ETKİLERİ**
...
Bu mekanizmaların olmadığı çalışmalarda hata yapılması ve verimsizlik
olması çok kuvvetli muhtemeldir.
...
Bir başka örnek olarak firmaların yapacağı yatırımlar kapsamı düşünülebilir. Geçmişte yapılan
yatırımlardaki bütçe çalışmaları, devreye alma süresi ve sürekliliği sağlama gibi veriler analiz edilir.
Gelecekte planlanan yatırımlara ne kadar harcanacağı ve nasıl olacağı konusu hakkında fikir sahibi
olunur ve süreç daha iyi bir hal alır.
...
Ayrıca önleyici aksiyonlar almanın birçok yöntemi vardır. Kaizenler, kök neden analizleri, 6sigma,
neden-neden analizleri kullanılabilir.
...
-   Karar verme süreçlerinde tahmin doğruluk için çok önemlidir. Gerçeğe yakın tahminlerin, daha
etkili kararlar alınmasını sağladığı belirtilmiştir. Ancak karar verme için kullanılan tahminler
yüzde yüz doğru olmak zorunda değildir.Nokta tahminler yanıltıcı da olabilmektedir.Veri
eksikliği veya hatalı verilerle çalışmanın, manuel veri işleme ve uzmanlık gereksinimini
artırdığı belirtilmiştir. Çünkü insanlar bu gibi durumlarda geçmiş tecrübelerini kullanabilirler. Nokta tahminler yanıltıcı da olabilmektedir. İnsan yönlendirmesinin fark yarattığı ve yorumlanabilirliğin karar verme süreçlerinde kritik bir faktör olduğu ifade edilmiştir. Modelin nasıl karar aldığına dair şeffaflığın sağlanması, özellikle yapay zeka destekli sistemlerin benimsenmesi açısından önemli bir konu olarak ele alınmıştır. Son olarak, reinforcement learning yaklaşımlarının karar verme süreçlerinde giderek daha kritik bir rol oynadığı belirtilmiştir. Bu yöntemler, çevresel geri bildirimlere dayanarak optimum kararları almak için kullanılan bir öğrenme stratejisi olarak öne çıkmakta ve özellikle robotik, finans ve otonom sistemler gibi alanlarda karar verme mekanizmalarını geliştirmede büyük bir potansiyel sunmaktadır.
Özellikle otonom sistemlerde
süreklilik çok önemlidir. Burada yapılacak ufak bir iyileştirme sistemin devamlılığını sağlamada
büyük rol oynar.
...
Son olarak veri alanında uzmanlaşmak için tavsiye olarak;
Bu işin yapılarak öğrenildiğinden ve çok fazla farklı uygulama içinde bulunmak gerektiğinden
bahsediliyor. Uzmanlaşacak olan kişinin mutlaka zaman ayırması gerekmektedir. Çok farklı
problemlerin içinde bulunmak o kişiyi geliştirecektir. Canlı  yarışmalar, güncel konular ile  insanlar için daha çekici olabilmektedir. En önemli olan noktalardan biri de bilen birisiyle usta-çırak
ilişkisiyle öğrenmektir. Bu veri alanında veya başka alanlarda da uzmanlaşmak için deneye deneye
yapılması gerektiği ve sonrasında ortaya çıkan problemlerin nasıl çözüleceğini bulmak gerekmektedir.
Bu işin içerisine girerken korkulmaması ve yapamam diye düşünülmemesi gerektiği tavsiye
edilmektedir.
